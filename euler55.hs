...
import Shared(isPalindromic,reverseNo)

addUntilPalindromic :: Integer -> Integer -> [Integer]
addUntilPalindromic x itToGo
                    | itToGo < 1 = [x]
                    | isPalindromic (sum' x) = [sum' x]
                    | otherwise = x : (addUntilPalindromic (sum' x) (itToGo-1))
                    where sum' x = x + (reverseNo x)

isLychrel :: Integer -> Bool
isLychrel x = length (addUntilPalindromic x 50) > 49

euler55 = length [x | x <- [10..9999], isLychrel x]

main = print euler55 -- correct - 249 in 0.712s